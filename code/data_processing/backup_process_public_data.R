...
load_rh4_data <- function(genome_info,gene_features_df,bin) {
    ...
    ret <- load_data("RH4",rh4_bam_paths[["k27ac"]],genome_info,gene_features_df,use_spike_in=T,bin=bin)

    ret$summarized_normalized_counts_df <- ret$summarized %>% mutate(logFC=RH4_treated - RH4_untreated)

    ret$rna_seq <- run_tximport( unlist(rh4_bam_paths[["rna-seq"]]) )
...
create_hct116_paths <- function() {
    ...
    for (lib in unique(sample_sheet_dt$lib)) {
